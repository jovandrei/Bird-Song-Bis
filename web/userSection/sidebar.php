<div id="sidebar" class="equal_height">
	<div class="widget-area">
		<ul><h3>Actions</h3>
			<div id="lang_sel">
    			<ul>
    				<li>
    					<a href="#" class="lang_sel_sel icl-en">
            				<span class="icl_lang_sel_current">Insert/Consult</span>
            			</a>
            			                                
            			<ul>
							
							<li class="icl-ru">          
								<a rel="alternate" hreflang="ru" href="#">
									<span class="icl_lang_sel_native"></span>
									<span class="icl_lang_sel_translated">
										<span class="icl_lang_sel_native">(</span>
										Add new users
										<span class="icl_lang_sel_native">)</span>
									</span>
								</a>
			                </li>
			                <li class="icl-ru">          
								<a rel="alternate" hreflang="ru" href="#">
									<span class="icl_lang_sel_native"></span>
									<span class="icl_lang_sel_translated">
										<span class="icl_lang_sel_native">(</span>
										Others
										<span class="icl_lang_sel_native">)</span>
									</span>
								</a>
			                </li>
			                
						</ul>
					</li>
				</ul>
			</div>
		</ul>
	</div>
	
	<div class="widget-area">
		<ul><h3>Insert/Consult</h3>
			<div class="textwidget">
				Switch between the tabs to fill the forms to add new elements, and consult the database.  
				
			</div>
		</ul>
	</div>
	
	<div class="widget-area">
		<ul><h3>Forms</h3>
			<ul>
				<li class="cat-item cat-item-297">
					<a href="_location.php" title="View all posts filed under Diary of a Language Teacher">
						Location
					</a>
				</li>
				
				<li class="cat-item cat-item-336">
					<a href="_analysis.php" title="View all posts filed under Language in the workplace">
						Analysis
					</a>
				</li>
				
				<li class="cat-item cat-item-14">
					<a href="_track_has_subject.php" title="View all posts filed under Teachers">
						Track has Subject
					</a>
				</li>
				
				<li class="cat-item cat-item-14">
					<a href="_track.php" title="View all posts filed under Teachers">
						Track
					</a>
				</li>
				
				<li class="cat-item cat-item-14">
					Others
					<ul class="children">
						<li class="cat-item cat-item-18">
							<a href="_subject.php" title="View all posts filed under Community">
								Subject
							</a>
						</li>
						<li class="cat-item cat-item-19">
							<a href="#" title="View all posts filed under Courses">
								Recording Hardware
							</a>
						</li>
						<li class="cat-item cat-item-19">
							<a href="#" title="View all posts filed under Courses">
								Marker
							</a>
						</li>
						<li class="cat-item cat-item-19">
							<a href="#" title="View all posts filed under Courses">
								Vegetation
							</a>
						</li>
					</ul>
				</li>
			</ul>
		</ul>
	</div>
</div><!--sidebar-->