...
hh("&#x00d7;
",1,200,'close').ttip=Calendar._TT["CLOSE"];
}
...
continue;
switch(b[i]){
case"%d":case"%e":d=parseInt(a[i],10);
break;
...
break;
case"%P":case"%p":if(/pm/i.test(a[i])&&hr<12)
hr+=12;
...
return this.sub(filter||Prototype.JSONFilter,'#{
1}
');
}
...
var Ajax={
_request:null,_requestObjects:[function(){
...
offset:0,startTime:0,endTime:0,loadTime:0,initialize:function(){
}
...
isEntryManager:false,isLiHidden:false,entry:'',initialize:function(isEntryManager,entry){
...
var value='';
switch(fieldType){
case'time':value=this.getTimeInputValue(columnId);
break;
case'eurodate':value=this.getEuroDateInputValue(columnId);
break;
case'date':value=this.getDateInputValue(columnId);
break;
case'phone':value=this.getPhoneInputValue(columnId);
break;
case'money':value=this.getMoneyInputValue(columnId);
break;
case'checkbox':value=this.getCheckboxInputValue(field);
break;
case'radio':value=this.getRadioInputValue(field);
break;
default:value=this.getSimpleInputValue(field);
break;
}
...
,isEntryManager:false,Rules:'',PublicForm:'',ConditionService:'',Entry:new Object(),RulesByConditionFieldName:new Object(),processAfterShowArray:new Object(),initialize:function(publicForm){
...
,runRulesOnload:function(){
if(!this.PublicForm.isEntryManager){
for(var colID in this.Rules){
if($(colID)){
this.process($(colID));
break;
}
...
entry:'',decimals:2,basePrice:0,currency:'',totalText:'',basePriceText:'',publicForm:'',merchantFields:[],hiddenClassNames:['rule_hide','hide'],tableTmpl:'<table id="run" border="0" cellspacing="0" cellpadding="0">'+'<tfoot>'+'#{
rows}
'+'</tfoot>'+'<tbody>'+'<tr><td colspan="2"><b>#{
totalText}
</b><span>#{
total}
</span></td></tr>'+'</tbody>'+'</table>',initialize:function(publicForm){
this.publicForm=publicForm;
...
__ENTRY=(typeof(__ENTRY)=='undefined')?{
}
:__ENTRY;
if(typeof(__PRICES)=='undefined'||!__PRICES){
__PRICES={
BasePrice:0,Currency:'&#36;
',MerchantFields:[]}
;
}
}
...
tplValues={
'totalText':this.totalText,'total':this.formatNumber(total),'rows':html}
;
...
fieldToPrice={
'fieldValue':fieldValue,'field':this.merchantFields[i]}
;
...
for(var i=0;
i<this.hiddenClassNames.length;
i++){
...
formLogic:new WufooFormLogic(),fieldLogic:new WufooFieldLogic(),runningTotal:'',ruleLogic:'',formHeight:'',timerActive:false,genericInputs:{
}
,sortedTabindexes:[],isEntryManager:false,unableToChangeFile:'We were unable to change your file.',initialize:function(runInit,isEntryManager){
this.isEntryManager=isEntryManager;
...
Calendar.setup({
inputField:"Field"+calendaFieldID,displayArea:"cal"+calendaFieldID,button:"pick"+calendaFieldID,ifFormat:"%B %e, %Y",onSelect:onSelectFunction}
);
}
}
,handleTabs:function(){
...
for(var i=0;
i<this.sortedTabindexes.length;
i++){
...
parameters:"entryId="+$('EntryId').value+"&fieldId="+fieldId+"&fileName="+file_name+"&formId="+formId,onComplete:function(r){
